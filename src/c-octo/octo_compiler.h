...
void octo_tok_list_insert(octo_list*dst,octo_list*src,int index){
  for(int z=0;z<src->count;z++) octo_list_insert(dst,octo_tok_copy((octo_tok*)octo_list_get(src,z)),index++);
}
...
octo_const* octo_make_const(double v,char m){octo_const*r=(octo_const*)calloc(1,sizeof(octo_const));r->value=v,r->is_mutable=m;                       return r;}
octo_reg  * octo_make_reg  (int v)          {octo_reg  *r=(octo_reg*)calloc(1,sizeof(octo_reg  ));r->value=v;                                       return r;}
octo_pref * octo_make_pref (int a,char l)   {octo_pref *r=(octo_pref*)calloc(1,sizeof(octo_pref ));r->value=a;r->is_long=l;                          return r;}
octo_proto* octo_make_proto(int l,int p)    {octo_proto*r=(octo_proto*)calloc(1,sizeof(octo_proto));octo_list_init(&r->addrs);r->line=l,r->pos=p;     return r;}
octo_macro* octo_make_macro()               {octo_macro*r=(octo_macro*)calloc(1,sizeof(octo_macro));octo_list_init(&r->args),octo_list_init(&r->body);return r;}
octo_smode* octo_make_smode()               {octo_smode*r=(octo_smode*)calloc(1,sizeof(octo_smode));                                                  return r;}
octo_flow * octo_make_flow (int a,int l,int p,char*t){octo_flow*r=(octo_flow*)calloc(1,sizeof(octo_flow));r->addr=a,r->line=l,r->pos=p,r->type=t;    return r;}
octo_mon  * octo_make_mon  ()               {octo_mon  *r=(octo_mon*)calloc(1,sizeof(octo_mon  ));                                                  return r;}
...
char* octo_intern_counted(octo_program* p,char* name,int length){
  ...
  if(p->strings_used+length+1>=OCTO_INTERN_MAX){
    return p->is_error=1, snprintf(p->error,OCTO_ERR_MAX,"Internal Error: exhausted string interning table."), (char*)"";
  }
...
void octo_append(octo_program*p, char byte){
  ...
  p->romLineMap[p->here] = p->source_line;
...
void octo_compile_statement(octo_program*p){
  ...
  if(octo_match(p,":"))octo_resolve_label(p,0);
    ...
    octo_reg*rh=(octo_reg*)octo_map_get(&p->aliases,octo_intern(p,"unpack-hi"));
    octo_reg*rl=(octo_reg*)octo_map_get(&p->aliases,octo_intern(p,"unpack-lo"));
...
octo_program* octo_program_init(char* text, int startAddress){
  ...
  memset(p->romLineMap, 255, OCTO_RAM_MAX*sizeof(uint32_t));
